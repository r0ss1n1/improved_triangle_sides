...
int main(void) {
	...
	printf("\n\n");
	Triangle * S = &FS;
	printf("Opposite / Hypotenuse: %f\nAdjacent / Hypotenuse: %f\nOpposite / Adjacent:   %f\n\n", S->opposite / S-> adjacent, S->adjacent / S->hypotenuse, S->opposite / S->adjacent);
	printf("\n\n");
	printf("Opposite / Hypotenuse Angle: %f\nAdjacent / Hypotenuse Angle: %f\nOpposite / Adjacent Angle:   %f\n\n", asin(S->opposite / S-> adjacent) * RADIANS_TO_DEGREES, acos(S->adjacent / S->hypotenuse) * RADIANS_TO_DEGREES, atan(S->opposite / S->adjacent) * RADIANS_TO_DEGREES);
	return 0;
}